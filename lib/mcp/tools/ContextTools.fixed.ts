...
/**
 * Context Tool Handlers - FIXED MCP Format
 * All handlers now return proper MCP format: { content: [{ type: "text", text: "..." }] }
 */
  ...
  try {
    ...
    if (args.use_vector_search !== false && args.topic) {
      ...
      const vectorConversations = vectorResults.map(result => ({
        id: result.id,
        userMessage: result.payload.content.split(' ').slice(0, 50).join(' '),
        aiResponse: result.payload.content.split(' ').slice(50).join(' '),
